...
if (isset($_GET['fromDate'])) {
    ...
    while ($row = mysqli_fetch_array($resultQueryProcess)) {
        ...
        $data["QUANTITY"] = $row['TOTAL_SALES'];
        $productWeightConf = $row['TYPE'] . "?" . $row['PRODUCT'] . "?" . $row['SIZE'] . "?" . $row['THICKNESS'];
        $data["WEIGHT"] = $row['TOTAL_SALES'] * getSalesValueFromArray($productWeightConf, $weightArr);
...
?>
    <script type="text/javascript">
        var sizeArray = <?php echo json_encode($resArrSales); ?>;
        var result = {};
    </script>
<?php
}
