...
if (isset($_GET['fromDate'])) {
    ...
    $accQry = "SELECT T.DATE, SPI.REFERENCE_NO, SPI.FOLLOWED_BY_PERSON, SPI.CUSTOMER_NAME, 
                    (SPI.TOTAL_AMOUNT - SPI.GST_AMOUNT) AS PRODUCT_VALUE, SPI.GST_AMOUNT, SPI.TOTAL_AMOUNT, T.TOTAL_AMOUNT AS PAID_AMOUNT, (SPI.TOTAL_AMOUNT - T.BALANCE) AS AMOUNT_RECEIVED, T.BALANCE, 
                    CASE WHEN T.BALANCE=0 THEN 'CLEARED' ELSE 'PENDING' END AS STATUS 
                    FROM sample_pi SPI, transaction T
                    WHERE 1 = 1 
    ...
    while ($row = mysqli_fetch_array($resultQueryProcess)) {
        ...
        $data['Product Value'] = $row['PRODUCT_VALUE'];
        $data['GST'] = $row['GST_AMOUNT'];
        $data['Total Value'] = $row['TOTAL_AMOUNT'];
        $data['Paid Amount'] = $row['PAID_AMOUNT'];
        $data['Total Amount Received'] = $row['AMOUNT_RECEIVED'];
                    ...
                    <div class="col-lg-12">
                        <section class="panel">
                            <header class="panel-heading">
                                Accounts
                                <button type="button" class="btn btn-primary pull-left" onclick="location.href = 'ar_home.php';" style="margin: 0.5em;">
        ...
        var buildTable = function(arr, divId) {
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                for (var colIndex = 0; colIndex < columns.length; colIndex++) {
                    ...
                    row$.append($('<td class="' + columnHeader + ' ' + arr[i][columns[10]] +'" />').html(cellValue));
                }
                $("#" + divId).append(row$);
            }
        }
