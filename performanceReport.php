...
while ($row = mysqli_fetch_array($queryRuns)) {
  ...
  $data["INPUT"] = $row['SUM(`TOTAL_INPUT`)'] * $size;
  $data["PROCESSED"] = $row['SUM(`PROCESSED`)'] * $size;
  $data["UNPROCESSED"] = $row['SUM(`UNPROCESSED_OUTPUT`)'] * $size;
...
<script type="text/javascript">
  ...
  dateArray = [...new Set(dateArray)]; // REMOVE THE DUPOLICATE ENTRIES IN DATEARRAY
  ...
  for(let i=0; i<productArr.length; i++) {
    if(productArr[i] != "TOTAL") {
      ...
      for(let j=0; j<productDetailsLength; j++) {
        input += productDetails[j]["INPUT"];
        processed += productDetails[j]["PROCESSED"];
        unProcessed += productDetails[j]["UNPROCESSED"];
      }
      let obj = {};
            ...
            <div class="col-lg-12">
                <section class="panel">
                    <header class="panel-heading">
                      Performance-Length-Report (<b><?php echo $machineName; ?></b>)
...
<script type="text/javascript">
   ...
   var buildTable = function(tableId, resArr) {
    ...
    $('#excelBtn').click(function () {
      productArrTableIds = [];
      let fileName = "Performance-Length-Report ("+ <?php echo '"'.$machineName.'"'; ?> + ").xls";
