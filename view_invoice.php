...
$sql = "SELECT * FROM `transaction_balance` T, `sample_pi` P 
            ...
            AND P.INVOICE_STATUS = 4 AND T.BALANCE = 0
            AND T.IS_PENDING_AMOUNT = 0 
            UNION 
            SELECT * FROM `transaction_balance` T, `sample_pi` P 
            WHERE 1 = 1 
            AND T.SAMPLE_PI_ID = P.`SAMPLE_PI_ID` 
            AND P.INVOICE_STATUS = 4
            AND T.IS_PENDING_AMOUNT = 1 
            AND T.BALANCE = 0 ORDER BY 1 DESC 
        ";
...
?>
                           <tbody>
                              <tr>
                                    ...
                                    <td style="color:#0c1211";><?php echo $row['DISPATCH_DATE']; ?></td>
