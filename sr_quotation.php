...
if (isset($_GET['fromDate'])) {
    ...
    $toDate = $_GET['toDate'];
    $resultQuery = "SELECT Q.*, SPI.INVOICE_STATUS FROM quotation Q LEFT JOIN sample_pi SPI ON Q.REFERENCE_NO = SPI.REFERENCE_NO WHERE Q.Q_DATE BETWEEN '$fromDate' AND '$toDate' ORDER BY Q_DATE DESC";
    ...
    while ($row = mysqli_fetch_array($resultQueryProcess)) {
        $data = array();
        $data["Date"] = $row['Q_DATE'];
        $data['Reference #'] = $row['REFERENCE_NO'];
        $data['Followed By'] = $row['FOLLOWED_BY_PERSON'];
        $data['Customer Name'] = $row['CUSTOMER_NAME'];
        $data['Customer Mobile'] = $row['CUSTOMER_MOBILE'];
        $data["Q_STATUS"] = $row['Q_STATUS'];
        $data["INVOICE_STATUS"] = $row['INVOICE_STATUS'];
        $data["Pi"] = "No";
        $data["PPI"] = "No";
        $data["Invoice"] = "No";
...
<script type="text/javascript">
    ...
    sizeArray.forEach(function(obj) {
        if (obj["Q_STATUS"] == "1" && obj["INVOICE_STATUS"] == null) {
            obj["Pi"] = "No";
            obj["PPI"] = "No";
            obj["Invoice"] = "No";
        } else if (obj["Q_STATUS"] == "0" && obj["INVOICE_STATUS"] == "0") {
            obj["Pi"] = "Yes";
            obj["PPI"] = "No";
            obj["Invoice"] = "No";
        } else if (obj["Q_STATUS"] == "0" && parseInt(obj["INVOICE_STATUS"]) <= 3) {
            obj["Pi"] = "Yes";
            obj["PPI"] = "Yes";
            obj["Invoice"] = "No";
        } else if (obj["Q_STATUS"] == "0" && obj["INVOICE_STATUS"] == "4") {
            obj["Pi"] = "Yes";
            obj["PPI"] = "Yes";
            obj["Invoice"] = "Yes";
        }
            ...
            for (var i = 0; i < arr.length; i++) {
                ...
                for (var key in rowHash) {
                    if ($.inArray(key, columnSet) == -1) {
                        columnSet.push(key);
                        if (key == "Q_STATUS" || key == "INVOICE_STATUS") {
                            continue;
                        }
