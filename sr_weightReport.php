...
if (isset($_GET['fromDate'])) {
    ...
    $resultQuery = "SELECT 
        PURCHASED_DATE, TYPE, SUM(weight) AS TOTAL_PURCHASE
        FROM `purchase_details_bill`  
        WHERE 1=1 
        AND TYPE = '$type' 
        AND PURCHASED_DATE BETWEEN '$fromDate' AND '$toDate' 
        GROUP BY PURCHASED_DATE, TYPE 
        ORDER BY PURCHASED_DATE, TYPE
    ";
    ...
    while ($row = mysqli_fetch_array($resultQueryProcess)) {
        $data = array();
        $data["DATE"] = $row['PURCHASED_DATE'];
        $data['TYPE'] = $row['TYPE'];
        $data["PUR WEIGHT"] = $row['TOTAL_PURCHASE'];
        $data["SCRAP"] = 0;
        $productionDateSet[$i] = $row['DATE'];
        $totalProductionArr[$i] = $row['DATE'];
    ...
    // print_r($weightArr);
    $salesSql = "SELECT RES.DISPATCH_DATE, SUM(RES.SOLD_WEIGHT) AS WEIGHT_SOLD FROM
                    (SELECT 
                    SPI.DISPATCH_DATE, SPID.PRODUCT, SPID.TYPE, SPID.SIZE, SPID.THICKNESS, SPID.QUANTITY AS TOTAL_SALES, (SPID.QUANTITY * PW.weight) AS SOLD_WEIGHT 
                    FROM sample_pi_details SPID, sample_pi SPI, product_weight PW, category C 
                    WHERE 1 = 1 
                    AND C.PRODUCT_ID = PW.product_id 
    ...
    while ($row = mysqli_fetch_array($resultQueryProcess)) {
        $data = array();
        $data[$row['DISPATCH_DATE']] = $row['WEIGHT_SOLD'];
        $salesDateSet[$i] = $row['DISPATCH_DATE'];
        ...
        do {
            ...
            if($queryCount == 0) {
                ...
                while ($row = mysqli_fetch_array($resultQueryProcess)) {
                    ...
                    $data["PROD WEIGHT"] = 0;
                    $data["STOCK"] = $row['TOTAL_WEIGHT'] - $beforeDaysSales;
                    $data["SALES"] = $salesQty;
                    ...
                    <div class="col-lg-12">
                        <section class="panel">
                            <header class="panel-heading">
                                Sales Vs Production Vs Stock Report Weight
