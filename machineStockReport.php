...
if (isset($_GET['machineId'])) {
    ...
    $resultQuery = "SELECT C.PRODUCT_NAME, M.TYPE, M.SIZE, M.thickness, M.MACHINE_STOCK AS OLD_STOCK, M.UNPROCESSED 
                    FROM machine_stock M, category C 
                    WHERE 1=1
                    AND C.PRODUCT_ID = M.CATEGORY_ID
                    AND M.MACHINE_ID = $machineId
                    AND M.TYPE = '$type'
                    ORDER BY M.TYPE, C.PRODUCT_NAME, M.SIZE, M.thickness";
    ...
    if ($result = mysqli_query($conn, $resultQuery)) {
        if (mysqli_num_rows($result) > 0) {
            while ($row = mysqli_fetch_array($resultQueryProcess)) {
                ...
                $data["Old Stock"] = $row['OLD_STOCK'];
                $data['Unprocessed'] = $row['UNPROCESSED'];
...
<body>
                    ...
                    <div class="col-lg-12">
                        <section class="panel">
                            <header class="panel-heading">
                                ...
                                <button type="button" class="btn btn-primary pull-left" onclick="location.href = 'machineStock.php';" style="margin: 0.5em;">
    ...
    <script type="text/javascript">
        ...
        $("#pdfBtn").click(function(e) {
            ...
            // let php = "" + <? echo "'" . str_replace("-", "/", $date) . "'"; ?> + "".toString();
