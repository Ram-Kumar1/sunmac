...
if (isset($_GET['fromDate'])) {
    ...
    $weightQry = "SELECT RES.DISPATCH_DATE, RES.REFERENCE_NO, SUM(RES.WEIGHT) AS WEIGHT FROM (
                    SELECT SPI.DISPATCH_DATE, SPI.REFERENCE_NO, (SPID.QUANTITY * PW.weight) AS WEIGHT
                    ...
                    GROUP BY RES.REFERENCE_NO
                    ORDER BY RES.DISPATCH_DATE DESC
                ";
    $weightArr = array();
    $resultQueryProcess = mysqli_query($conn, $weightQry);
    while ($row = mysqli_fetch_array($resultQueryProcess)) {
        $weightArr[$row['REFERENCE_NO']] = $row['WEIGHT'];
    }
    ...
    while ($row = mysqli_fetch_array($resultQueryProcess)) {
        ...
        $data['Weight (KG)'] = $weightArr[$row['REFERENCE_NO']];
