...
$resultQuery = "SELECT C.PRODUCT_NAME, M.MACHINE_NAME, R.`UP_INPUT`, R.`GIVEN_INPUT`, R.`TOTAL_INPUT`, R.`GIVEN_OUTPUT`, R.`PROCESSED`, R.`OLD_STOCK`, R.`TOTAL_PROCESSED`, R.`TOTAL_STOCK`, R.`UNPROCESSED_OUTPUT`, R.`LAST_UPDATED_DATE`, R.`TYPE`, R.`SIZE`, R.thickness, TR.minutes FROM `production_stock_report` R, `category` C, `machine_details` M, `machine_time_record` TR WHERE 1=1 AND C.PRODUCT_ID=R.CATEGORY_ID AND M.MACHINE_ID=R.MACHINE_ID AND C.PRODUCT_ID=TR.CATEGORY_ID AND M.MACHINE_ID=TR.MACHINE_ID AND R.TYPE=TR.type AND R.SIZE=TR.size AND R.thickness=TR.thickness AND R.LAST_UPDATED_DATE=TR.date AND TR.CATEGORY_ID=R.CATEGORY_ID AND TR.MACHINE_ID=R.MACHINE_ID AND R.`LAST_UPDATED_DATE`='$date'";
...
while ($row = mysqli_fetch_array($resultQueryProcess)) {
  ...
  $data["TOT"] = $row['UP_INPUT'] + $row['GIVEN_INPUT'];
...
<center><h3>Data Entered On (<?php echo $date; ?>)</h3></center>
